...
std::array<bool, 8> sBox::transform(std::array<bool, 8> sequence, Box option) {
    ...
    // Dividing of 8-bit data into two 4-bit nibbles
    for (int i = 0; i < 4; i++) {
        leftNibble[i] = sequence[i];
        rightNibble[i] = sequence[i + 4];
    }
    int leftPosition = 0, rightPosition = 0, multiplier = 1, leftNumber, rightNumber;
    // Converting of binary numbers to decimal numbers for finding necessary indices in S-box
    for (int i = 4; i > 0; i--) {
        for (int j = i - 1; j > 0; j--) {
            multiplier *= 2;
        }
    ...
    result.fill(false);
    // Converting of decimal left nibble into binary 4-bit number
    while (leftNumber > 0) {
    ...
    i = 7;
    // Converting of decimal right nibble into binary 4-bit number
    while (rightNumber > 0) {
